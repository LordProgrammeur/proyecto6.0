...
class Producto {
    ...
    public static function obtenerPedidos($dbConnection) {
        // Consulta combinada con INNER JOIN
        $sql = "
            SELECT pedidos.*, usuario.nombre AS nombre_cliente 
            FROM pedidos 
            INNER JOIN login ON login.id = pedidos.id_cliente 
            INNER JOIN usuario ON usuario.correo = login.email
        ";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $pedidos[] = $row; // A adir cada fila al array
            }
        }
